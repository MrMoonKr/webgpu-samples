...
const TextRenderingMsdf: () => JSX.Element = () =>
  makeSample({
    name: 'Text Rendering - MSDF',
    description: `This example uses multichannel signed distance fields (MSDF) to render text. MSDF
      fonts are more complex to implement than using Canvas 2D to generate text, but the resutling
      text look smoother while using less memory than the Canvas 2D approach, especially at high
    ...
    sources: [
      ...
      },
      {
        ...
        contents: require('!!raw-loader!./msdfText.ts').default,
      },
      {
        name: './msdfText.wgsl',
        contents: msdfTextWGSL,
        editable: true,
      },
      {
...
export default TextRenderingMsdf;
